SELECT ID, FROM_UNIXTIME(FIRST_SEEN), FROM_UNIXTIME(LAST_SEEN) FROM SS_RRSET ORDER BY id DESC LIMIT 3;
...
SELECT COUNT(ID) FROM SS_RRSET WHERE YEAR(FROM_UNIXTIME(LAST_SEEN)) = 2019 AND MONTH(FROM_UNIXTIME(LAST_SEEN)) = 07 AND FIRST_SEEN!=LAST_SEEN;
...
SELECT ID,FROM_UNIXTIME(SEEN),SET_ID,EXP_SET_ID from SS_RRSET_EXP_REL WHERE YEAR(FROM_UNIXTIME(SEEN)) = 2019 AND MONTH(FROM_UNIXTIME(SEEN)) = 8 AND EXP_SET_ID is not null;
